...
export default async function LettersArchive({ params }: Props) {
  ...
  // Querying "letters" type from Sanity
  ...
  const letters = await client.fetch(query);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 md:gap-16">
          {letters?.length > 0 ? (
            letters.map((letter: any, index: number) => {
              // Staggered rotations for the collage/scrapbook look
                  ...
                  <Link href={`/${locale}/letters/${letter.slug || letter._id}`} className="block">
                    ...
                    <div className="space-y-4 px-2">
                      ...
                      <div className="pt-4 flex items-center gap-4">
                         <div className="h-px w-8 bg-stone-800 group-hover:w-12 group-hover:bg-white transition-all duration-500" />
                         <span className="text-[10px] font-mono text-stone-600 uppercase tracking-widest">Open File</span>
                      </div>
                    </div>
                  </Link>
                </motion.div>
