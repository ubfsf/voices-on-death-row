...
export default function Typewriter({
  ...
  return (
    <motion.span
      ...
      className={className}
      // Screen readers get the full text in one pass; the per-word
      // animation spans below are hidden from the accessibility tree.
      role="text"
      aria-label={text}
    >
      {elements.map((el: string, index: number) => {
        ...
        if (/^\s+$/.test(el)) {
          return <span key={index} aria-hidden="true">{el}</span>;
        }
