...
export default function SplitHero({ title, text, imageSide = "right" }: SplitHeroProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 min-h-[80vh] border-y border-stone-200 bg-paper">
      {/* Text Side */}
      ...
      {/* Image Side */}
      <div className={`relative bg-stone-200 overflow-hidden min-h-100 ${imageSide === "right" ? "order-2" : "order-1"}`}>
        <img 
        ...
        <div className={`absolute inset-y-0 bg-linear-to-r from-paper to-transparent w-32 hidden md:block ${imageSide === "right" ? "left-0" : "right-0 rotate-180"}`} />
      </div>
    </div>
  );
}
